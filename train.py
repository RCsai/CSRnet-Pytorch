...
if __name__ == "__main__":
    ...
    criterion = nn.MSELoss(reduction="sum")  # objective
    ...
    min_mae_epoch = -1
    print("开始训练", "\n")
    for epoch in range(1, cfg.epochs + 1):  # start training
        ...
        print("epoch_" + str(epoch) + "_loss:", epoch_loss / len(train_dataloader))
        ...
        with torch.no_grad():
            ...
            if epoch_mae < min_mae:
                ...
                torch.save(model.state_dict(), os.path.join(cfg.checkpoints, "best" + ".pth"))  # save checkpoints
            print('Epoch_', epoch, ':   MAE: ', epoch_mae, ' Min MAE: ', min_mae, ' Min Epoch: ',
                  min_mae_epoch)  # print information
            cfg.writer.add_scalar('Val_MAE', epoch_mae, epoch)
            cfg.writer.add_image(str(epoch) + '/Image', denormalize(image[0].cpu()))
            cfg.writer.add_image(str(epoch) + '/Estimate density count:' + str('%.2f' % (et_densitymap[0].cpu().sum())),
                                 et_densitymap[0] / torch.max(et_densitymap[0]))
            cfg.writer.add_image(str(epoch) + '/Ground Truth count:' + str('%.2f' % (gt_densitymap[0].cpu().sum())),
                                 gt_densitymap[0] / torch.max(gt_densitymap[0]))
